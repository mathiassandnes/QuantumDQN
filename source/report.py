...
def preprocess_results():
    ...
    for trial_folder in os.listdir(path):
        ...
        for run_folder in os.listdir(os.path.join(path, trial_folder)):
            ...
            df['environment'] = config['environment']['name']
...
def plot_score(episodes, description, kind, environment, include_epsilon=False, show_min_and_max=False):
    if kind == 'evaluation':
...
def plot_acrobot_episode(episodes, observations, predictions, row):
    observations = pd.DataFrame(observations,
                                columns=['cos_theta1', 'sin_theta1', 'cos_theta2', 'sin_theta2', 'angular_velocity_theta1',
...
if __name__ == '__main__':
    episodes, description = preprocess_results()
